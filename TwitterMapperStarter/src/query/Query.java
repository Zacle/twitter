...
public class Query implements Observer{
    ...
    // Marker that displays on the map
    ...
    public void terminate() {
        for (MapMarker m : map.getMapMarkerList()){
            if (m.getLayer().getName().equals(this.layer.getName())){
                m.getLayer().setVisible(false);
    ...
    public void update(Observable observable, Object o) {
        if (this.filter.matches((Status) o)){
            this.marker = new MapMarkerDisplay(this.layer, this.queryString, Util.statusCoordinate((Status)o), color, (Status)o);
