...
public class Application extends JFrame {
    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                map().setToolTipText(getToolTip(pos, p));
    ...
    private String getToolTip(ICoordinate pos, Point p){
        double pixel = pixelWidth(p);
        List<MapMarker> list = getMarkersCovering(pos, pixel);
        String ans = "";
        String image;
        for (MapMarker m: list){
            if (m instanceof MapMarkerDisplay){
                MapMarkerDisplay md = (MapMarkerDisplay)m;
                image = md.getUser().getBiggerProfileImageURL();
                ans += String.format("<html><body><img src='%s'>"
                        + md.getStatus().getText() + "</body></html>", image);
            }
        }
        return ans;
    }
